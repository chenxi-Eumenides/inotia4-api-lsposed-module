...
> **现行端点以 `docs/api-reference.md` 为准**，分级依据见 `docs/api-reference.md` §八（op 域）。
...
| `SAVE_Save` | 0x129600 | `int (void)` | **完整静默保存**（无参！之前"[x0+0x8c0]"是误判——0x2f3000+0x8c0 是全局上下文指针非参数）。内部 SV_GoldGet/StatPoint/SkillPoint 校验 → KEY_ResetActive → 细分 SaveInformation/Player/CharacterAll/Inventory/Quest/Event/ETC → SAVE_SaveData 写盘；校验失败返回 0。API save（v0.4.16，静默无弹窗） |
| `GAMESTATE_SetState` | 0x151590 | `void (int32_t state)` | **游戏状态机切换**（STATE_nState @0x307492：4=主菜单 5=world；state==4 分支 GAME_Exit(0x100334)+STATE_Set(4)+Enter 回调）。API main-menu 回主菜单（v0.4.17，纯 API 无 UI 依赖，真机验证 world→main_menu 无崩溃） |
...
| `CHAR_SetSkillUsage` | 0xe4cc0 | `void (void* ch, int32_t onoff)` | 写 [ch+0x3a0] bit0-2（AI 技能开关 0-7）。API skill-usage（v0.4.10，frida hook 验证写入生效） |
| `MERCENARYSYSTEM_Release` | 0x118ab4 | `void (int32_t mercenarySlot)` | **佣兵遣散**：FindAsMercenarySlot 找角色 → MERCENARYGROUPSKILLSYSTEM_Remove(0x118900) 移除队伍技能 → 角色 +0x352=-1 → 清 +0x3cc 标志 → CHAR_SetSituation(ch,5) → MERCENARYSLOT_Initialize 重置槽 → GAMESTATE_SetState。⚠️ 参数是 +0x352 槽 ID（非 mercenary 端点 slot 索引）；无该槽角色时静默返回（FindAsMercenarySlot null 分支），API 前置 `mercenary not found`（v0.4.8） |
