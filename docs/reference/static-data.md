...
- `field_catalog.json`：从 48 条扩展到 **83 条**（新增 12 条装备表字段，2026-08-08 分布分析 + 文本引用交叉验证）
- 覆盖 23 张表：含 ITEMGRADEBASE(4)、ITEMRARITYGRADEBASE(1)、ITEMSTATICBASE(2)、ITEMCLASSBASE(4)、MONDATABASE(4)、ITEMDATABASE(2)、QUESTINFOBASE(2)、MAPINFOBASE(2)、CHARCLASSBASE(4)、MERCENARYINFOBASE(2)、NPCINFOBASE(1)、MONSTERDROPBASE(2)、ITEMDESCBASE(2)、ITEMENCHANTBASE(2)

### 7.5 附魔属性对照表（P2，2026-08-08 改版作者 xlsx 解析）

> 来源：`~/Documents/Install/Android/Game/艾诺迪亚4_盗版大修_v1.3.2_20260704/附魔属性对照表1.xlsx`（改版作者提供，与盗版大修附魔系统对应）。
...
- 元素系（火=17/风=18/寒气=19/神圣=20/黑暗=21/毒=22/重力摆=23/冰霜=24/治愈气息=25/狂战士=26/瞬间回复=27/魔力专家=28/减少敌意值=29）：有 1 级/105 级成长值（如瞬间回复 1.1→415、冰霜 1.1→41.5）
- 抗性系（眩晕/睡眠/失明/恐惧/减速/沉默抗性=30-35）：1 级=105 级=1.0

**子表 2：附魔特殊属性词条表**（总分 0-128）
- 总加分 → 附魔词条：0-128 按 8 系循环（火/风/寒气/神圣/黑暗/毒/重力摆/冰霜/治愈气息/狂战士/瞬间回复/魔力专家/减少敌意值——每 13 分一轮循环）
- 示例：0=火、1=风、2=寒气、13=火、26=火...

**子表 3：完美附魔抗性词条表**（总分 0-128）
- 总加分 → 抗性词条：按 6 抗性循环（眩晕/睡眠/失明/恐惧/减速/沉默抗性）
- 示例：0=眩晕抗性、1=睡眠抗性、6=眩晕抗性...

**子表 4：实际抗性表**（0-16）
- 实际抗性：0=减速抗性、1=沉默抗性、2=眩晕抗性、3=睡眠抗性、4=失明抗性、5=恐惧抗性（6 抗性循环）

> **交叉验证提示**：子表 2/3 的"总加分"即附魔系统中装备同类型属性值之和（与 §7.2 ITEMENCHANTBASE 字段、改版说明"4 宝石触发附魔/同类型属性派生"对应）。
