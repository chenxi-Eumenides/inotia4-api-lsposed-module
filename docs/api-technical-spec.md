...
> ⚠️ **v0.5.0 更新**：API 已按 7 域分组（character/world/item/quest/ui/system/op），本表历史端点路径的旧前缀（/api/action/*、/api/info/*、/api/data/*）均已废弃，完整新旧对照见 `docs/api-reference.md` 第十一节。
...
| 宝石镶嵌 | 镶嵌宝石到插槽 | `ITEMSYSTEM_PutJewel`/`ApplySocket`、`UIMix_FindJewelUpgradeStuff`——**✅ 已实现（v0.4.6 /api/item/inventory/{role}/jewel，镶嵌后手动消耗宝石防刷）** | P0 |
...
| 操作 | 游戏内方式 | 函数证据 | 优先级 |
|---|---|---|---|
| 分配属性点 | 属性面板加点 | `CHAR_GetStatMain`(0xdb9f0)+`CHAR_SetStatMain`(0xdf1c4)（读/写 [ch+0x256+i*2]）+`CHAR_SetStatusPoint`——**✅ 已实现（v0.4.5 /api/character/grow/{role}/stat，属性+1/能力点-1，StatDivide 语义绕过 UI 缓冲，真机验证力量11→12/精力15→16/能力点耗尽拦截）** | P0 |
...
| 佣兵遣散 | 佣兵菜单「解散」 | `UIMercenary_ButtonDischargeExe` → `MERCENARYSYSTEM_Release`——**✅ 已实现（v0.4.8 /api/character/party/discharge）** | P0 |
| 佣兵取出/放置 | 佣兵列表管理 | `CHAR_UnequipItemToInven`（对佣兵角色）——**✅ 已实现（v0.4.9 /api/character/party/withdraw）** | P2 |
...
| 操作 | 游戏内方式 | 函数证据 | 优先级 |
|---|---|---|---|
| 购买 | 商店买物品（金币） | `DEALSYSTEM_FindSaleByID`+`ITEM_GetBuyPrice`+`INVEN_SaveItem`——**✅ 已实现（v0.4.14 /api/item/shop/buy，绕过 cursor 纯函数链）** | P0 |
...
| 操作 | 游戏内方式 | 函数证据 | 优先级 |
|---|---|---|---|
| NPC 对话/交互 | 走到 NPC 旁按交互键 | `PLAYER_DoCheckNearNPC`+`UINpc_InitNPC`+`UINpc_ExeCurrentNpcTask`——**✅ 已实现（v0.4.13，现路径 /api/ui/dialog/interact、/api/ui/dialog/select + GET /api/ui/dialog/content）** | P0 |
...
| 放弃任务 | 任务菜单「放弃」 | `QUESTSYSTEM_Find`+`RemoveSlot`——**✅ 已实现（v0.4.15 /api/quest/quit，通用非硬编码）** | P0 |
...
> **2026-08-08 新增 OP 项**（完整端点见 `docs/api-reference.md` §8.2）：
...
> **2026-08-08 全量盘点定稿**：游戏中全部操作按系统分组（结构见 `docs/api-reference.md` §0），合法 38 端点 + OP 16 端点。
...
> **2026-08-08 结构定稿**：操作按系统拆分 12 类别（movement/combat/inventory/character/party/npc/ui/shop/quest/save/craft + op）；**v0.5.0 起端点按 7 域分组（详见 `docs/api-reference.md` §0/第十一节）**。
...
| P0 | 商店买（shop/buy） | ✅ v0.4.14 已实现（DEALSYSTEM 表定位+BuyPrice+SaveItem+MinusMoney，绕过 cursor）；GET /api/item/shop/items 商品列表 |
