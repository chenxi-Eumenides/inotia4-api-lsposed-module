...
| 未开始 | **BFS 导航问题：被怪物阻挡时重规划失败卡死 + 重规划无日志** | 2026-08-14 真机2 实测（影子丛林2 map31 回退移动）：玩家多次停在怪旁不动，move_to 无法继续。**代码根因**（game_ops_action.cpp:139-194 `nav_task_tick` 撞墙重规划分支）：① 重规划 BFS 用 `use_units=true`（`nav_unit_blocks` 把怪物所在格标记阻挡，game_nav.cpp:38-60）——怪物挡路时目标方向被怪格封死；② 两个重规划分支（resume 格 BFS / 当前格 BFS）均严格要求 `np.found==true` 才采用路径——`found=false`（不可达）时直接 `return false` 终止任务，**不用 best_tile 最近可达点接续**；③ 失败即终止，无等待/重试（v0.4.51 注释"不再多余尝试"），怪物动态移动但玩家不等它走开。**附带确认**：BFS 为纯 4 方向无斜向（game_nav.cpp:98/162 `for d<4`）；重规划路径**无任何 MOVE_LOG**（宏已定义但移动/重规划路径未使用），无法从日志观测重规划行为。**变体根因（2026-08-14 补充）**：模块瓦片建模（静态 bit3 + nav_unit_blocks）与游戏引擎 CHAR_Move 碰撞判定不一致——出口 tile (36,26) 模块判可走、引擎 CHAR_Move 撞墙（影子丛林2 出口实测卡 2 次，重试后成功），「模块判可走/引擎判阻挡」的 tile 使重规划永远跳不出 → replan≥5 终止卡死 | 重规划分支在 `found=false` 时改用 np 的 `nearest_x/nearest_y`（最近可达点）继续走；或撞墙后等待 N 帧/周期重试（让动态怪移开）；或重规划时排除玩家行进方向前 N 格的单位阻挡；补 MOVE_LOG 重规划日志（撞墙/探测/成功/失败各阶段）；核查模块瓦片建模与引擎碰撞判定差异（CHAR_GetBlock 返回值语义），对齐阻挡建模 | 本会话 2026-08-14 用户报告 + 代码分析 |
| 未开始 | **BFS 是否应把尸体纳入阻挡（研究验证）** | 2026-08-14 用户澄清：**敌人尸体（hp=0）不影响行走**（引擎 CHAR_Move 语义）。**代码事实**：`nav_unit_blocks`（game_nav.cpp:38-60）过滤条件仅 type 0-2/status≤2/坐标有效/非英雄，**无 hp 过滤**——hp=0 尸体（type=1,status=1 仍在角色池）会被标记阻挡 → 模块 BFS 把尸体当阻挡与引擎不一致，疑为出口卡住诱因之一（尸体占用出口相邻格时 BFS 判不可达 → 重规划失败）。**待验证**：引擎是否真的完全无视尸体（碰撞判定 CHARSYSTEM_GetCharacterBlock 对 hp=0 对象返回值）；模块 BFS 加入/排除尸体对路径规划的影响；尸体格应否从 nav_unit_blocks 排除 | frida 实测 CHAR_GetBlock 对 hp=0 对象返回值；nav_unit_blocks 加 hp>0 过滤后真机验证路径规划（对比卡死场景）；确认引擎碰撞语义后同步修正 | 本会话 2026-08-14 用户报告 |
...
| 未开始 | **units distance 语义定义错误（对象 tile 恒不可达，需改为紧贴距离）** | 2026-08-14 用户权威澄清：**地图对象本身均不可达**（单位 tile 被自身占位标记阻挡），当前 `build_units_json`（game_read.cpp:381-396）用 `depth_map[对象tile]` 作 distance 恒为 -1（`nav_bfs_multi` 的 `nav_unit_blocks` 已把单位格标阻挡），仅剩 nearest_distance 有值——与「distance=到达对象路径长度」的文档语义不符。**期望语义**：① `distance` = 玩家到「能紧贴该对象的位置」（对象相邻可达格）的 BFS 路径长度；② 若紧贴位置全部不可达 → `distance=-1`；③ **「紧贴」的几何定义需实测**——指角色碰撞体紧贴对象（非 tile 中心距离 1），需真机验证对象相邻格 vs 碰撞半径的判定边界（相关：BFS 为 4 方向无斜向，紧贴位置即上下左右相邻可达格） | 修正 units 距离计算：distance = min(对象 4 邻可达格 depth_map)；全不可达 → -1（保留 nearest_distance）；实测确认「紧贴」判定（角色碰撞 vs 相邻 tile 中心），必要时按碰撞半径修正相邻格集合；同步修正 api-reference/game-guide 的 distance 字段说明 | 本会话 2026-08-14 用户报告 |
...
| 未开始 | **新增 debug 端点暴露 BFS 寻路（返回完整路线）** | 2026-08-14 用户要求（排查 P0 #5 系列导航问题的观测手段）：现有 `/api/world/map/distance` 只返回距离（found/nearest/distance），无法观测 BFS 实际路径、阻挡判定、单位占用。**需求**：新增 debug 端点（如 `GET /api/debug/path?tx=&ty=` 或类似），直接调用 `nav_bfs` 返回**完整路线**——起点/终点 tile、`found`、路径点序列（tile 坐标或像素）、`nearest` 最近可达点、以及阻挡信息（各 tile 的静态阻挡/单位占用标记），便于真机对比「模块 BFS 判定 vs 引擎 CHAR_Move 实际碰撞」（P0 #5 变体）、尸体阻挡影响（P0 尸体条目）、重规划探测（resume 格）行为 | 实现 debug 端点：Kotlin controller + native JNI 桥接（参考现有 distance 端点 data 链路）；native 侧复用 nav_bfs/nav_unit_blocks，输出路径 dirs 转换的 tile 序列 + 每步阻挡标记；路由登记 api-reference §debug 域 | 本会话 2026-08-14 用户报告 |
| 未开始 | **API 出口坐标与游戏实际切图区域不一致（出口坐标错误）** | 2026-08-14 真机2 实测（影子丛林2 map31 出口）：玩家站在 API 出口 tile **(36,26)/(37,26)** 上并移动（584,424→600,424，横跨两出口 tile），**切图不触发**（map 仍 31）。**根因**：切图判定 = `map_link_check`（game_ops_action.cpp:66-71）调用游戏引擎 `fn_go_map_link_by_char(ch, tx, ty)`——引擎判定这些 tile **无地图链接**；而 **API 出口来自静态瓦片 bit7=1 扫描**（game_read.cpp:290-303，`tiles[yy*64+xx] & 0x80`）——两套数据源不一致。**推论**：API 出口列表（含 map/exits、切图用）是静态 bit7 推测，非引擎 `GoMapLinkByChar` 实际链接点；此前切图成功是 move_to 路径恰好经过引擎真实链接 tile。**用户判断**：当前位置实际不属于出口（无切图即证） | 逆向 `GoMapLinkByChar`（F_GO_MAP_LINK 相关符号）判定逻辑：读哪张链接表/条件；用 frida 或 debug 端点遍历当前地图全部 tile 调 GoMapLinkByChar 找出**引擎真实切图 tile**；API exits 改为引擎链接点数据源（或修正瓦片 bit7 语义）；同步修正切图依赖（move_to/walk_dir 的目标应指向引擎链接点） | 本会话 2026-08-14 用户报告 |
