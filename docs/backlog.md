...
| ✅ **性能优化（减少计算/读取，提高 API 响应）** | 大响应端点（/api/data/map/list、ITEMDATABASE、text、events）数据量大，响应耗时长 | **✅ v0.4.57-59 缓存层完成**（architecture §2.3）：**v0.4.57** 帧计数驱动采集；**v0.4.58** 改惰性+帧边界（请求驱动，无常驻线程）；**v0.4.59** 双模式（interval>0 预取 / interval=0 惰性，表驱动一行切换）；真机并发 500 全成功 169 req/s，预取槽命中缓存 avg 10ms。剩余：静态表端点（map/list/ITEMDATABASE/text）数据量大仍为实时读取，可后续按需缓存 | 原 P1 提升至 P0（2026-08-12），v0.4.57-59 完成 |
