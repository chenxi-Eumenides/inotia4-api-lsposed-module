...
| ✅ **性能优化（减少计算/读取，提高 API 响应）** | 大响应端点（/api/data/map/list、ITEMDATABASE、text、events）数据量大，响应耗时长 | **✅ v0.4.57-58 惰性帧同步缓存层完成**（architecture §2.3）：**v0.4.57** 帧计数驱动采集 + 表驱动缓存槽；**v0.4.58** 改为**惰性 + 帧边界**（无常驻线程，请求驱动：新鲜返回缓存/过期等帧边界构造 + refreshing 单飞 + 等帧锁外化），currentMap unitsJson 去重；真机 10 客户端并发 500 请求全成功 218 req/s（v0.4.57 338 req/s，惰性等帧代价换零空闲负担）。剩余：静态表端点（map/list/ITEMDATABASE/text）数据量大仍为实时读取，可后续按需缓存 | 原 P1 提升至 P0（2026-08-12），v0.4.57-58 完成 |
