...
| 状态 | 待办项 | 现状 / 卡点 | 需要的探索 / 实现 | 来源 |
|---|---|---|---|---|
| ✅ 2026-08-12 | **瓦片矩阵入静态数据（逆向地图数据文件格式）** | ✅ **2026-08-12 完成**（详见 data-sources.md §3.2）：完整逆向文件格式（base layer + MAP_LoadLayer 5 u16 + sections + exit count + exits 6 bytes）；离线解析脚本 `scripts/parse/export_map_tiles.py` 产出 `static-data/json/maps/tiles.json`（416 图，2.2MB base64）；**真机 frida 验证 m31 bit3/bit6/bit7 100% 一致**（9 个 exit 坐标与 runtime/API 完全匹配，仅剩 6 cells bit5 map link area 标记差异 0.15%，不影响寻路）。**剩余**：① bit 5（link area 区域）逆向（可选，需 MAP_SetEventAreaOn 0x112ca4 反汇编）；② 游戏需在 world 状态时跑 `scripts/analyze/dump_all_map_tiles.py` 验证 416 图；③ 接入 native 层（v0.4.61 当前 interval=0 惰性，从静态数据读取） | 用户 2026-08-12 指定 P0 |
