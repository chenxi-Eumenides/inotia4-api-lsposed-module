...
| 状态 | 待办项 | 现状 / 卡点 | 需要的探索 / 实现 | 来源 |
|---|---|---|---|---|
| 未开始 | **瓦片矩阵入静态数据（逆向地图数据文件格式）** | 瓦片通行矩阵 `*(*(base+0x2f3f48))`（64×64，bit3=阻挡 bit7=出口）是**静态数据**（同图不变），当前运行时读内存；MAPINFOBASE 静态表仅含元数据（name/textId/u16），不含瓦片矩阵 | ① 逆向地图数据文件格式（MAP_Load(0x1149d4) 读哪个文件/格式）→ 按 mapId 一次性解析入静态数据（416 图 × 4KB ≈ 1.7MB base64）；② 若逆向不出：游戏中遍历切图采集全部地图瓦片存静态数据；③ 完成前瓦片保持惰性获取（v0.4.61 已改 interval=0），完成后改从静态数据读取 | 用户 2026-08-12 指定 P0 |
