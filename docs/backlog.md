...
| 未开始 | **BFS 是否应把尸体纳入阻挡（研究验证）+ 瓦片建模 vs 引擎碰撞差异根治** | 2026-08-14 用户澄清：**敌人尸体（hp=0）不影响行走**（引擎 CHAR_Move 语义）。**代码事实**：`nav_unit_blocks`（game_nav.cpp:38-60）过滤条件仅 type 0-2/status≤2/坐标有效/非英雄，**无 hp 过滤**——hp=0 尸体（type=1,status=1 仍在角色池）会被标记阻挡 → 模块 BFS 把尸体当阻挡与引擎不一致，疑为出口卡住诱因之一（尸体占用出口相邻格时 BFS 判不可达 → 重规划失败）。**待验证**：引擎是否真的完全无视尸体（碰撞判定 CHARSYSTEM_GetCharacterBlock 对 hp=0 对象返回值）；模块 BFS 加入/排除尸体对路径规划的影响；尸体格应否从 nav_unit_blocks 排除。**v0.5.27 真机补充（本会话，由原 BFS 重规划卡死条目变体根因归并）**：影子丛林1 map30 实测发现多处「模块判可走/引擎判阻挡」的**假可走格**（如 tile (30,30)/(31,29)/(34,38)，模块静态 tiles 判可走、引擎 CHAR_Move 撞墙），导致玩家被困 1×1 死角反复撞墙——与尸体阻挡同属「瓦片建模 vs 引擎碰撞差异」家族（含 type=2 装饰物碰撞体覆盖相邻格等）。v0.5.27 已做缓解（撞墙格临时排除 wall-block），根治需逆向 CHARSYSTEM_GetCharacterBlock 对齐阻挡建模 | frida 实测 CHAR_GetBlock 对 hp=0 对象/type=2 装饰物/假可走格的返回值；nav_unit_blocks 加 hp>0 过滤 + 假可走格对齐引擎碰撞后真机验证；逆向 CHARSYSTEM_GetCharacterBlock 完整碰撞语义，对齐模块瓦片/单位阻挡建模 | 本会话 2026-08-14 用户报告 + v0.5.27 真机 |
