...
| 未开始 | **改版机制：背包销毁=出售（70% 价格）** | 用户 2026-08-09 告知：该改版将**背包「销毁」改为「出售」，可获得物品原价 70%**。API 的 discard（丢弃）与 sell（出售）语义需对齐此机制（discard 是否也应得 70% 金币？） | 核实 discard 在游戏内的真实行为（是否实为出售得金币）；API discard/sell 语义与文档对齐 | 用户 2026-08-09 告知 |
| 未开始 | **discard 未对齐改版机制（销毁按钮=出售）** | 2026-08-09 实测对比（同物品护胸甲）：`sell` 得金币 **+460**（=原价 657×70%）；`discard` 金币 72866 无变化（纯删除）。**用户说明**：改版将物品本身「销毁」按钮指向了**出售**（得 70% 价格），游戏当前可能**无真正的销毁按钮入口**——API discard 对应的游戏内操作实为出售。**后续验证：sell API 与在商人处出售价格是否一致** | discard 改造：销毁时按 ITEM_GetPrice×70% 入账金币（对齐改版「销毁按钮=出售」）；验证 sell 与商人出售价一致（shop 场景） | 用户 2026-08-09 告知 |
