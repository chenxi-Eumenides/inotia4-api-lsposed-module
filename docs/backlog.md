...
| 状态 | 待办项 | 现状 / 卡点 | 需要的探索 / 实现 | 来源 |
|---|---|---|---|---|
| ✅ 2026-08-12 | **瓦片矩阵入静态数据（逆向地图数据文件格式）** | ✅ **2026-08-12 完成**（详见 data-sources.md §3.2）：MAP_Load(0x1149d4) 文件格式已逆向（width=byte2, height=byte3, 11-bit tile ID 编码 + 阻挡 tile ID 列表 + 64×64 matrix 写入）；离线解析脚本 `scripts/parse/export_map_tiles.py` 已产出 `static-data/json/maps/tiles.json`（416 图，2.2MB base64）；**真机 frida 验证 m31 一致率 99.73%**（11 字节 diff 全在 exit 区域，offline parser 未处理 exit data，base layer 100% 正确）。**剩余**：游戏需在 world 状态时跑 `scripts/analyze/dump_all_map_tiles.py` 验证 416 图 + 接入 native 层（v0.4.61 当前 interval=0 惰性，从静态数据读取） | 用户 2026-08-12 指定 P0 |
