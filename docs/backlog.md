...
| ✅ v0.5.15 | **VMA 治理：裸 VMA 归拢 + dlsym 化改造** | ✅ **2026-08-14 v0.5.15 完成**：①裸 VMA 全量归拢（代码级 8 处 0x1506d8/0x14b858 → F_PANEL_WIPEOUT_ENTER/F_PANEL_NPC_QUEST_ENTER，注释级裸地址更新，补登记 F_CHAR_GET_BLOCK_VMA）；②新增 symbol_resolver.h/.cpp **自研 ELF 内存解析**（PT_DYNAMIC p_offset 定位 + SysV hash 查找 + SHN_UNDEF/STT_TLS 过滤 + PT_LOAD 范围校验防同名撞库，规避 Android linker namespace 隔离，无 dlsym 依赖）；③symbol_registry.h X-macro 注册表（v0.5.14 135 条 → v0.5.15 **204 条**，自动化扫描补全 70 条漏网符号：F_PANEL_*_ENTER 27 个/G_INVEN/G_PLAYER_ACTIVE/G_POPUP_*/G_UICHOICE_*/MAXLEVELBASE_*/MERCENARYINFOBASE_*/G_CHARLOC_*/G_NPCTASKLIST_*/F_CREATE_ITEM 等，ELF 解析覆盖率 83%）单一来源，check_symbols.py 同步从注册表提取；④bridge_init 接入 resolver.attach，112 处 F_* 函数指针 ELF 解析优先、VMA 保底回退（g_symbol_report 记录 source）；⑤**GOT 槽 RELATIVE 反查闭环**（v0.5.15）：resolveSlot 从 .rela.dyn 按 r_offset 反查 R_AARCH64_RELATIVE addend，42 个无符号名 GOT 槽 RELATIVE 命中优先、VMA 仅兜底；**8 版本（20260704~0810）全部 42/42 精确匹配、204 符号 0 漂移**（盗版大修版本改内容不改代码，地址全稳定） | 已完成（真机验证待 backlog 完成标准：需真机启动确认游戏正常） | 用户 2026-08-12 指定 P1 |
