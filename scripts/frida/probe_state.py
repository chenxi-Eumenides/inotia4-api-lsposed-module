import frida
import sys

PKG = "com.com2us.inotia4.normal.freefull.google.global.android.common"

js = r"""
var base = null;
Process.enumerateModules().forEach(function (m) {
    if (m.name === 'libgame.so') base = m.base;
});
if (!base) { console.log('LIBGAME_NOT_FOUND'); }
else {
    // STATE_nState @0x307492（UI 状态机）
    var st = base.add(0x307492).readU8();
    console.log('STATE_nState=' + st);
    // GAMESTATE_nState @0x72b068
    var gst = base.add(0x72b068).readU32();
    console.log('GAMESTATE_nState=' + gst);
    // INITSTATE_nState @0x72b06d
    var ist = base.add(0x72b06d).readU8();
    console.log('INITSTATE_nState=' + ist);
}
"""


def main():
    dev = frida.get_usb_device(timeout=10)
    pid = int(sys.argv[1]) if len(sys.argv) > 1 else None
    session = dev.attach(pid if pid else PKG)
    script = session.create_script(js)
    script.on("message", lambda msg, data: print(msg["payload"] if msg["type"] == "send" else msg))
    script.load()
    sys.stdin.read()


if __name__ == "__main__":
    main()
