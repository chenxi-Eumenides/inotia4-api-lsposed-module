 ...
 * 收口 nativeSetStackLimitEnabled/nativeSetJewelBatchMix/nativeSetTilesData 三处直调：
 * 启动期走 [applyToNative]，配置端点变更走 [applyOnChange]，瓦片矩阵加载走 [applyTiles]。
 */
interface ConfigApiService {
    /** 启动期全量应用：堆叠 patch + 宝石批量合成 + 静态瓦片矩阵（内部自行判断 NativeBridge.ready） */
    fun applyToNative()

    /** 配置变更增量应用：仅当对应值变化时通知 native（POST /api/config/set 用） */
    fun applyOnChange(oldStack: Boolean, oldJewel: Boolean)
...
class ConfigApiServiceImpl : ConfigApiService {
    ...
    override fun applyOnChange(oldStack: Boolean, oldJewel: Boolean) {
        if (!NativeBridge.ready) return
